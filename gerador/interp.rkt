...
(define (read-value env var type size gerador)
  (begin
     (display "Variável: ")
     (displayln (var-id var))
     (display "Tipo:  ")
     (displayln (type-name type))
     (display "Tamanho: ")
     (displayln (value-value size))
  ))
...
(define (eval-stmt env s gerador)
  (match s
    [(input (var v) type size)
        (read-value env v type size gerador)]
...
(define (eval-stmts env blk gerador)
  (match blk
    ['() env]
    [(cons s blks) (let ([nenv (eval-stmt env s gerador)])
                       (eval-stmts nenv blks gerador))]))



(define (imp-extendido-interp prog)
  (eval-stmts (make-immutable-hash) prog gerador))

(provide imp-extendido-interp eval-expr)
